...
const DynamicAndamanItineraryPage = () => {
  ...
  const sliderImages = [
    {
      ...
      description: "Discover the capital city with its rich history, colonial architecture, and vibrant culture."
    },
    {
      ...
      description: "Experience the world-famous Radhanagar Beach and untouched natural beauty."
    },
    {
      ...
      description: "Unwind on serene beaches and explore the peaceful charm of this hidden gem."
    },
    {
      ...
      description: "Journey through mangrove creeks to discover ancient limestone caves and tribal culture."
    }
  ];
  ...
  const fetchItinerary = async () => {
    try {
      ...
      const response = await fetch(`/api/admin/destinations/andaman/itineraries?packageId=${params.packageId}&t=${timestamp}`, {
      ...
      if (response.ok) {
        ...
        console.error('Failed to fetch itinerary, status:', response.status);
      ...
      <div className="min-h-screen bg-white">
        ...
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-4">{error || 'No itinerary found for this package'}</p>
            <button
              onClick={() => router.push('/destination/andaman')}
              className="bg-orange-500 text-white px-6 py-3 rounded-lg hover:bg-orange-600 transition-colors duration-200"
            >
              Back to Andaman Packages
            </button>
          </div>
        </div>
        <Footer />
...
export default DynamicAndamanItineraryPage;